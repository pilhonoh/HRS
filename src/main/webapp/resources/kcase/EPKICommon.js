(function () {
    ...
    if(K_OSName == "Windows"){
        ...
        file = "http://" + url + "/client/Windows/KSignCASE_For_HTML5_Windows_v1.3.13.exe";
    }
    else if(K_OSName == "Mac") {
        kcaseLibUrl = "//" + url + "/kcase/lib/mac_n_linux_lib";
        file = "http://" + url + "/client/MacOS/HTML5_KCaseAgentForMac.pkg";
    }
    else if(K_OSName == "Linux") {
        kcaseLibUrl = "//" + url + "/kcase/lib/mac_n_linux_lib";
        file = "http://" + url + "/" + "client/Linux/KCaseAgentForLinux-1.3.9-1.el6.i386.rpm";
    }
    ...
    var failAgentService = function() {
        alert("처리 프로그램(KCaseAgent)이 설치되어 있지 않습니다. 안내에 따라 설치하고 본 창을 새로고침(F5)한 후에 다시 시도해 주시기 바랍니다.");
        location.href = file;
    };
